...
load("./Bunya_mixing/random_mixing_random_20_2_5.RData")
den_sum_pois = random
load("./Bunya_mixing/crest_mixing_crest_21_201_16.RData")
den_sum_crest = crest
load("./Bunya_mixing/crest_mixing_slope_22_201_16.RData")
den_sum_slope = slope
## recompile
...
data_plot$stand = data_plot$mean / data_plot$no_col
...
str(data3)
data4 = data3
data_filtered <- data4 %>% filter(!is.na(mean_fertilized))
start_grid <- expand.grid(a = seq(min(data_filtered$mean_fertilized, na.rm = TRUE),
                                  max(data_filtered$mean_fertilized, na.rm = TRUE), length.out = 5),
...
## threshold
density_threshold <- data_filtered %>%
  filter(mean_fertilized < 10) %>%
  arrange(density) %>%
  slice(1) %>%
  pull(density)
if (!is.na(density_threshold)) {
  cat("Density at which mean_fertilized falls below 10:", density_threshold, "\n")
} else {
  cat("No density found where mean_fertilized < 10.\n")
}
################### 
